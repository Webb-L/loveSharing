            ...
            <form role="form" action="{{route('role.store')}}" method="post">
                @if ($errors->any())
                    @foreach ($errors->all() as $error)
                        @if($error == '1')
                            @component('admin.component.callout',['type'=>'success','content'=>'创建成功！'])
                            @endcomponent
                        @else
                ...
                <div class="row">
                    <div class="col-sm-12">
                        ...
                        <div class="form-group">
                            <label>角色名：</label>
                            <input type="text" name="role_name" class="form-control" placeholder="">
                        </div>
                    </div>
                </div>
    ...
    <script>
        $(() => {
            ...
            $('.col-sm-4').click(function (e) {
                let flag = 0;
                $(this).siblings().map((i, t) => {
                    if ($($(t).children().children()[0]).is(':checked')) {
                        flag++;
                        if (flag == $(this).siblings().length) {
                            $($($(t).parent().parent().parent().children()[0]).children()[0]).attr('checked', 'checked');
                        } else {
                            $($($(t).parent().parent().parent().children()[0]).children()[0]).removeAttr('checked', 'checked');
                        }
                    }
                })
            });
